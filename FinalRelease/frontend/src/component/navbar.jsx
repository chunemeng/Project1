...
export default function NavBar({user}) {
    ...
    const navItems = [{label: "首页", value: "/"}, {label: "任务", value: "/task"}, {label: "众包", value: "/worker"}, {label: "需求", value: "/publish"}];
    ...
    const items = [{
        key: '0', label: (<p>{"用户名：" + user?.nickname}</p>),
    }, {
        key: '1', label: (<div onClick={() => {
            navigate("/profile")
        }}> 个人主页</div>),
    }, {
        key: '2', label: (<div onClick={try_logout}> 退出登录</div>),
    }];
    const navbar = useMemo(() => {
        return <div className={"flex-raw"} style={{minWidth: "1500px"}}>
            {contextHolder}
