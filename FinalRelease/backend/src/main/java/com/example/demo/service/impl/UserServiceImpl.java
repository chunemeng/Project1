...
public class UserServiceImpl implements UserService {
    ...
    public Result login(LoginDto loginDTO, HttpServletResponse httpServletResponse) {
        ...
        Optional<AuthDto> user = userRepository.findAuthByUsername(username);
        if (user.isPresent()) {
    ...
    @Override
    public Result register(RegiDto loginDTO) {
        String username = loginDTO.getUsername();
        String password = loginDTO.getPassword();
        if (username == null || password == null) {
            return Result.error("网络错误");
        ...
        newUser.setCover("/male1.png");
        newUser.setNickname("用户" + username);
